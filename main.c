...
// посимвольное получение динамической строки с пробелами c получением её длины
char *get_string(int *len) {
    *len = 0; // изначально строка пуста
    ...
    // читаем символы, пока не получим \n
    while (c != '\n') {
        s[(*len)++] = c; // заносим в строку новый символ

        // если размер больше размера контейнера, то увеличим его размер
        if (*len >= capacity) {
...
// получение динамической строки для флага с
